...
% training for each sample point, this model is called SQR. In this example
% instead of using random quantile levels, we select quantile levels from
% inadequately learned quantile levels. We determine this quantile levels
% using a threshold value (epsilon). This model is called Adaptive SQR (ASQR)
%
% we introduce a condition $C : E >= epsilon$, where $\epsilon$
% denotes a threshold hyperparameter.
%
%NOTE: alpha_generator function is used for selection of quantile levels.
%
%
%
%
...
alpha_rev = 0;
eps = 0.005; % Threshold value to be used
...
%             [~, ~, yPred_opp_val] = GT2_fismodel_LA1_new(Val.inputs, number_mf, number_inputs,number_outputs,length(Val.inputs), Learnable_parameters, output_membership_type,input_membership_type,input_type,type_reduction_method,u, opp_alpha_val, delta, alpha_rev);
    ...
    alpha = alpha_generator(total_min_values, total_scaling_values, total_cell, rest, error, resultArray);
